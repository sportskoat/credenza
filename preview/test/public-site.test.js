...
describe("the public site shares one look and one header", () => {
  ...
  it("resolves every token it uses", () => {
    ...
    for (const { rel, html } of DOCS) {
      ...
      // A token can also be set on the element itself. /landing/ passes each
      // photo to its lens that way: style="--lens-img: url('…')". Those are
      // defined, just not in a stylesheet.
      for (const attr of html.matchAll(/style="([^"]*)"/g)) {
        for (const m of attr[1].matchAll(/(--[a-z0-9-]+)\s*:/g)) local.add(m[1]);
      }
