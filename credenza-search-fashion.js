...
export function serializeAskCandidates(query, items, { limit = 25, now = Date.now() } = {}) {
  return selectAskCandidates(query, items, limit).map((item) => ({
    ...
    seller: clamp(item.seller, FIELD_LIMITS.seller),
    batch: clamp(item.batch, FIELD_LIMITS.batch),
    size: clamp(item.size, FIELD_LIMITS.size),
    colorway: clamp(item.colorway, FIELD_LIMITS.colorway),
    agentLink: clamp(item.agentLink, 240),
    findSource: clamp(item.findSource, 240),
    findStatus: normalizeFindStatus(item.findStatus),
